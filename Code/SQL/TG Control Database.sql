...
-- VIEW: Facilita a autenticação, retornando o tipo de usuário.
CREATE OR REPLACE VIEW `vw_user_login` AS SELECT u.email, u.passwordHASH, CASE WHEN t.email IS NOT NULL THEN CASE WHEN t.is_coordinator = 1 THEN 'PROFESSOR_TG' ELSE 'PROFESSOR' END WHEN s.email IS NOT NULL THEN 'ALUNO' ELSE 'NAO_AUTENTICADO' END AS tipoUsuario FROM `user` u LEFT JOIN `teacher` t ON u.email = t.email LEFT JOIN `student` s ON u.email = s.email WHERE u.status = 'Active';

-- VIEW: Reúne detalhes do aluno e sua turma.
CREATE OR REPLACE VIEW vw_student_details AS SELECT s.email, u.FirstName, u.LastName, CONCAT(u.FirstName, ' ', u.LastName) AS nomeCompleto, u.profile_picture_url, u.status AS user_status, s.advisor_email, s.class_disciplina, s.class_year, s.class_semester, s.estagio_tg_atual, CONCAT(c.disciplina, ' - ', c.year, '/', c.semester) AS turma_descricao FROM student s JOIN user u ON s.email = u.email JOIN class c ON s.class_disciplina = c.disciplina AND s.class_year = c.year AND s.class_semester = c.semester;

-- VIEW: Reúne detalhes do professor.
CREATE OR REPLACE VIEW vw_teacher_details AS SELECT t.email, u.FirstName, u.LastName, CONCAT(u.FirstName, ' ', u.LastName) AS nomeCompleto, u.profile_picture_url, u.status AS user_status, t.is_coordinator, CASE WHEN t.is_coordinator = 1 THEN 'Coordenador TG' ELSE 'Professor/Orientador' END AS tipoProfessor FROM teacher t JOIN user u ON t.email = u.email;

-- VIEW: Calcula estatísticas agregadas por turma.
CREATE OR REPLACE VIEW vw_class_summary AS WITH StudentTGProgress AS ( SELECT s.email AS student_email, s.class_disciplina, s.class_year, s.class_semester, COALESCE(AVG(CASE WHEN t.status = 'completed' THEN 1.0 ELSE 0.0 END), 0.0) AS progresso_individual FROM student s LEFT JOIN task t ON s.email = t.student_email GROUP BY s.email, s.class_disciplina, s.class_year, s.class_semester ) SELECT c.disciplina, c.year, c.semester, COUNT(DISTINCT stp.student_email) AS numero_alunos, SUM(CASE WHEN stp.progresso_individual = 1.0 THEN 1 ELSE 0 END) AS tgs_concluidos, AVG(stp.progresso_individual) AS progresso_medio_turma FROM class c LEFT JOIN StudentTGProgress stp ON c.disciplina = stp.class_disciplina AND c.year = stp.class_year AND c.semester = stp.class_semester GROUP BY c.disciplina, c.year, c.semester;

-- VIEW: Visão completa das tarefas para o dashboard do Coordenador TG.
CREATE OR REPLACE VIEW `vw_professortg_dashboard_tasks` AS WITH StudentProgress AS ( SELECT `student_email`, COUNT(*) AS total_tasks, SUM(CASE WHEN `status` = 'completed' THEN 1 ELSE 0 END) AS completed_tasks FROM `task` GROUP BY `student_email` ), LatestSubmission AS ( SELECT ts.`student_email`, ts.`sequence_order`, ts.`submission_timestamp`, ROW_NUMBER() OVER(PARTITION BY ts.`student_email`, ts.`sequence_order` ORDER BY ts.`submission_timestamp` DESC) as rn FROM `task_submission` ts ), LatestReview AS ( SELECT tr.`student_email`, tr.`sequence_order`, tr.`submission_timestamp`, tr.`status`, tr.`review_timestamp`, ROW_NUMBER() OVER(PARTITION BY tr.`student_email`, tr.`sequence_order`, tr.`submission_timestamp` ORDER BY tr.`review_timestamp` DESC) as rn FROM `task_review` tr ) SELECT CONCAT(u.FirstName, ' ', u.LastName) AS nomeAluno, s.email AS emailAluno, c.disciplina AS turma_disciplina, c.year AS turma_year, c.semester AS turma_semester, s.estagio_tg_atual AS estagio_aluno, t.sequence_order, t.title AS task_title, t.status AS task_status, t.estagio_task, (COALESCE(sp.completed_tasks, 0) / NULLIF(sp.total_tasks, 0)) AS progresso_geral_aluno, CASE WHEN t.status = 'completed' THEN 'Aprovado' WHEN lr.status IS NOT NULL THEN lr.status WHEN ls.submission_timestamp IS NOT NULL THEN 'Pendente' ELSE '---' END AS status_revisao_ultima_submissao, ls.submission_timestamp AS timestamp_ultima_submissao, lr.review_timestamp AS timestamp_ultima_revisao, s.advisor_email FROM `task` t JOIN `student` s ON t.student_email = s.email JOIN `user` u ON s.email = u.email JOIN `class` c ON s.class_disciplina = c.disciplina AND s.class_year = c.year AND s.class_semester = c.semester LEFT JOIN StudentProgress sp ON s.email = sp.student_email LEFT JOIN LatestSubmission ls ON t.student_email = ls.student_email AND t.sequence_order = ls.sequence_order AND ls.rn = 1 LEFT JOIN LatestReview lr ON ls.student_email = lr.student_email AND ls.sequence_order = lr.sequence_order AND ls.submission_timestamp = lr.submission_timestamp AND lr.rn = 1;

-- VIEW: Unifica submissões e revisões recentes para o feed de atividades do Professor TG.
CREATE OR REPLACE VIEW vw_professortg_recent_activity AS SELECT 'SUBMISSION' AS event_type, ts.student_email, ts.submission_timestamp AS event_timestamp, s.class_disciplina, s.class_year, s.class_semester, CONCAT(u.FirstName, ' ', u.LastName) AS nomeCompleto, CONCAT('Enviou a tentativa ', ts.attempt_number, ' para "', t.title, '"') AS acao_descricao FROM task_submission ts JOIN task t ON ts.student_email = t.student_email AND ts.sequence_order = t.sequence_order JOIN student s ON ts.student_email = s.email JOIN user u ON ts.student_email = u.email UNION ALL SELECT 'REVIEW' AS event_type, tr.student_email, tr.review_timestamp AS event_timestamp, s.class_disciplina, s.class_year, s.class_semester, CONCAT(u.FirstName, ' ', u.LastName) AS nomeCompleto, CONCAT('Teve a seção "', t.title, '" ', CASE tr.status WHEN 'approved' THEN 'aprovada' WHEN 'revision_requested' THEN 'marcada para revisão' ELSE 'revisada' END, ' pelo orientador') AS acao_descricao FROM task_review tr JOIN task_submission ts ON tr.student_email = ts.student_email AND tr.sequence_order = ts.sequence_order AND tr.submission_timestamp = ts.submission_timestamp JOIN task t ON ts.student_email = t.student_email AND ts.sequence_order = t.sequence_order JOIN student s ON tr.student_email = s.email JOIN user u ON tr.student_email = u.email;
...
CREATE TRIGGER trg_unlock_next_task_on_complete AFTER UPDATE ON task FOR EACH ROW BEGIN DECLARE next_sequence INT; DECLARE next_task_estagio INT; DECLARE student_current_estagio INT; IF NEW.status = 'completed' AND OLD.status <> 'completed' THEN SET next_sequence = NEW.sequence_order + 1; SELECT estagio_task INTO next_task_estagio FROM task WHERE student_email = NEW.student_email AND sequence_order = next_sequence; IF next_task_estagio IS NOT NULL THEN SELECT estagio_tg_atual INTO student_current_estagio FROM student WHERE email = NEW.student_email; IF next_task_estagio = student_current_estagio THEN UPDATE task SET status = 'in_progress' WHERE student_email = NEW.student_email AND sequence_order = next_sequence AND status = 'locked'; END IF; END IF; END IF; END //

-- TRIGGER: Desbloqueia a primeira task do novo estágio ao promover aluno, se a última do anterior estiver completa.
CREATE TRIGGER trg_unlock_tasks_on_stage_promotion AFTER UPDATE ON student FOR EACH ROW BEGIN DECLARE previous_stage_last_task_status ENUM('locked', 'in_progress', 'completed'); DECLARE new_stage_first_task_sequence INT; IF NEW.estagio_tg_atual > OLD.estagio_tg_atual THEN SELECT status INTO previous_stage_last_task_status FROM task WHERE student_email = NEW.email AND estagio_task = OLD.estagio_tg_atual ORDER BY sequence_order DESC LIMIT 1; IF previous_stage_last_task_status = 'completed' THEN SELECT MIN(sequence_order) INTO new_stage_first_task_sequence FROM task WHERE student_email = NEW.email AND estagio_task = NEW.estagio_tg_atual; IF new_stage_first_task_sequence IS NOT NULL THEN UPDATE task SET status = 'in_progress' WHERE student_email = NEW.email AND sequence_order = new_stage_first_task_sequence AND status = 'locked'; END IF; END IF; END IF; END //

-- TRIGGER: Marca a task como 'completed' quando uma revisão é inserida com status 'approved'.
CREATE TRIGGER trg_complete_task_on_approval AFTER INSERT ON task_review FOR EACH ROW BEGIN IF NEW.status = 'approved' THEN UPDATE task SET status = 'completed' WHERE student_email = NEW.student_email AND sequence_order = NEW.sequence_order AND status <> 'completed'; END IF; END //
